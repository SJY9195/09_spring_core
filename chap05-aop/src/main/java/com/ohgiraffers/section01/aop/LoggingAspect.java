...
public class LoggingAspect {
    ...
    * */
    ...
    public void logPointCut(){
        System.out.println("이거 진짜 실행 안되나?");  //이건 진짜 service(joinpoint)지정만 해준거라 println은 출력되지 않는다! 굳이 println은 없어도 된다!
    }

    @Before("LoggingAspect.logPointCut()")  //LoggingAspect는 없어도 된다 ( 같은클래스이므로) 그냥 보여주려고 쓴것!
...
//        }
    ...
    * */
    ...
    public Object logAround(ProceedingJoinPoint joinPoint) throws Throwable {
        ...
        // 원본 메소드 실행
        Object result = joinPoint.proceed();  //이게 있어야 원본 메소드가 실행된다! 없으면 실행 x
        if(result != null && result instanceof Map){
